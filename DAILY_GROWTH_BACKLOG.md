...
|---:|---|---|---|---|---|---|---|---|---|
| 1 | DRSA-008 | RED | Restore the Little Doggy Rescue partner destination | On 2026-07-22 the LDR homepage and `/safe-dog-rehoming/` repeatedly returned HTTP `520`; the QUIC.cloud error states its node cannot connect to the origin. Cache-busted, `?page_id=261`, root, www, HTTP and HTTPS requests all reproduced the failure. Dog Rescue SA links to this host from five pages, so the small-dog route is currently unavailable. | High | High | Medium | High | Cross-network repair only: inspect LDR origin availability and QUIC.cloud origin configuration; do not change Dog Rescue SA destinations without approval |
| 2 | DRSA-007 | RED | Add the missing self-referencing canonical to `/breeds/` | Weekly crawl on 2026-07-20 verified that all seven sitemap URLs return `200` without `noindex`, but `/breeds/` alone has no canonical tag; the other six canonicals match their live URLs. The source file also lacks the tag. This is an index-signal inconsistency on a new page, but canonical changes require explicit approval. | High | High | Low | Medium | After explicit approval, add only `<link rel="canonical" href="https://dogrescuesa.co.za/breeds/">`, deploy through the confirmed direct Pages project and verify live |
